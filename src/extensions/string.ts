...
// https://stackoverflow.com/questions/43439317/ionic-2-angular-2-global-import-extension-methods
// tried declare global no success, also worked for a while with .d.ts, and suddently when pulling project
// a new from git it stopped working, well wtf.
...
String.prototype.lowerCaseFileExtension = function(this: string) {
  const dotExtension = this.match(/\.\w+$/);

  if (!dotExtension) {
    return this;
  }

  const filename = this.substr(0, dotExtension.index);

  return filename + dotExtension[0].toLocaleLowerCase();
};
